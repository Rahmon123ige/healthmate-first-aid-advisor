...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ user }) => {
  ...
  const saveToHistory = (newMessages: Message[]) => {
    ...
    const updatedHistory = [...allHistory, ...newMessages.slice(-2)]; // Save last 2 messages (user + AI)
    localStorage.setItem(`healthmate_history_${user.email}`, JSON.stringify(updatedHistory));
  };

  const getUrgencyLevel = (content: string): 'low' | 'medium' | 'high' => {
...
Format your response naturally, but include an urgency assessment.`
  ...
  const findNearbyClinicss = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          const { latitude, longitude } = position.coords;
          
          // Mock clinic data - in a real app, you'd use Google Places API
          const mockClinics = [
            {
    ...
    <div className="chat-container">
      ...
      {showClinics && (
        <div className="clinic-finder">
          ...
          <button className="find-clinics-button" onClick={findNearbyClinicss}>
